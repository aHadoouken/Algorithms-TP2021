...
int CheckGum(const IGraph &graph, const std::vector<int>& path){
    std::vector<int> verts = graph.GetNextVertices(path[0]);
    std::vector<int> checkedVerts(graph.VerticesCount(), 0);
    checkedVerts[path[0]]++;

    for (int i = 1; i < path.size(); i++){
        auto it = std::find(verts.begin(), verts.end(), path[i]);
        if (it == verts.end())
            return 0;
        checkedVerts[path[i]]++;
        verts = graph.GetNextVertices(path[i]);
    }
    for (auto v: checkedVerts){
        if (v == 0)
            return 0;
    }
    return 1;
